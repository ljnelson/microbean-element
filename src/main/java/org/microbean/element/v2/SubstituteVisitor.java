...
final class SubstituteVisitor extends StructuralTypeMapping<Void> {
  ...
  private final List<TypeVariable> to;
  ...
  private final SubstituteVisitor with(final List<? extends TypeVariable> from,
                                       final List<? extends TypeVariable> to) {
  ...
  public final ExecutableType visitExecutable(ExecutableType e, final Void x) {
    ...
    */
    ...
    if (anyMatches(this.to, typeVariables, Equality::equalsIncludingAnnotations)) {
      e = DefaultExecutableType.withTypeVariables(e, newInstances(typeVariables));
    ...
    // Visit the other "parts" of the method that are structurally
    // relevant.
    ...
    return DefaultExecutableType.withTypeVariables(visitedE, visitedTypeVariables);
  }
  ...
  public final IntersectionType visitIntersection(final IntersectionType t, final Void x) {
    ...
    return DefaultIntersectionType.of(list);
  }
  ...
  public final TypeMirror visitTypeVariable(final TypeVariable tv, final Void x) {
    ...
    if (!(tv instanceof SyntheticCapturedType)) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (Equality.equalsNotIncludingAnnotations(f, tv)) {
  ...
  public final WildcardType visitWildcard(final WildcardType wt, final Void x) {
    ...
    if (superBound == null) {
      ...
      return DefaultWildcardType.upperBoundedWildcardType(visitedExtendsBound, wt.getAnnotationMirrors());
      ...
      return DefaultWildcardType.lowerBoundedWildcardType(visitedSuperBound, wt.getAnnotationMirrors());
    } else {
  ...
  // A port/translation of Types#newInstances(List).  "perform
  // alpha-renaming of free-variables in 't' [free variables in tvs]"
  //
  // This is a slavish port for the most part. There are several cases
  // where we could make things more efficient but the undocumented
  ...
  private final List<? extends TypeVariable> newInstances(final List<? extends TypeVariable> tvs) {
    ...
    for (int i = 0; i < visitedTvs.size(); i++) {
      ...
      // Compute a new upper bound for it.  Note the usage of tvs and
      // visitedTvs in the from and to slots respectively.
      final TypeMirror visitedUpperBound = visitor.visit(upperBound); // RECURSIVE

      // It *seems* that if upperBound and visitedUpperBound are
      // identical we wouldn't have to do anything below.  But the
      // compiler's version of this method always creates a new
      // instance, and the method is named newInstances, so we do too.

      // Create a new TypeVariable whose upper bound is the
      // just-visited upper bound.
      final DefaultTypeVariable newTv = DefaultTypeVariable.withUpperBound(tv, visitedUpperBound);

      // Replace the type variable at position i (the very one we're
      // looking at) with the new, newly-bounded type variable.
  ...
  private final List<? extends TypeVariable> visitUpperBoundsOf(final List<? extends TypeVariable> tvs) {
    ...
    // Phase 1 (it appears): effectively call visit() on the upper
    ...
    // Phase 2: Create a list of type variables temporarily without
    // upper bounds.  We will give them upper bounds in phase 3.
    final List<DefaultTypeVariable> newTvs = new ArrayList<>(tvs.size());
    for (final TypeVariable tv : tvs) {
      newTvs.add(DefaultTypeVariable.withUpperBound(tv, null));
    }

    // (All Lists at this point are the same size.)

    // Phase 3: Perform substitution over the visited (phase 1) upper
    // bounds of the caller-supplied type variables with the
    // un-upper-bounded type variable "prototypes" created in phase 2.
    // (This will result in type variables that have no upper bound
    // yet, but we have them in a list, and so can doctor them in
    // phase 4 below.)
    ...
    // Phase 4: Replace the unspecified upper bounds (see phase 2)
    // with the substituted bounds we calculated in phase 1 and
    // doctored in phase 3.
    for (int i = 0; i < newTvs.size(); i++) {
      // newTvs.set(i, DefaultTypeVariable.withUpperBound(newTvs.get(i), visitedUpperBounds.get(i)));
      newTvs.set(i, newTvs.get(i).withUpperBound(visitedUpperBounds.get(i)));
    }
  ...
  private static final boolean anyMatches(final Collection<? extends TypeMirror> ts,
  ...
  // Does at least one s in ss "pass the test" represented by p when p
  // is invoked with t and s?
